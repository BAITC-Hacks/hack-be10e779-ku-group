...
import { gain, KIND_COLOR, KIND_LABEL, type LeadGroup } from './model'
    ...
    <div className={`q-contrast q-contrast-${props.size}`} aria-labelledby={headId} role="group">
      <div className="q-contrast-stat">
        <div className="eyebrow" id={headId}>
          Сутки {g.lead} · средняя ошибка MAE, % номинала
        </div>
        {vsCurve != null ? (
          <p className={`q-stat ${vsCurve < 0 ? 'is-worse' : ''}`}>
            ...
            <span className="q-stat-cap">{vsCurve >= 0 ? 'точнее' : 'хуже'} кривой мощности</span>
          </p>
        ) : (
          <p className="muted small">В метриках нет пары «модель — кривая мощности» для этого горизонта.</p>
        )}
        {vsPersist != null && (
          <p className="q-stat-sub">
            и на <b className="mono">{pct(Math.abs(vsPersist))}</b> {vsPersist >= 0 ? 'точнее' : 'хуже'} персистентности
          </p>
        )}
      </div>

      <ul className="q-bars" aria-label={`MAE по методам, сутки ${g.lead}, % номинала`}>
        {g.rows.map((r) => (
          <li key={r.name} className={`q-bar q-bar-${r.kind}`}>
            <span className="q-bar-label" title={r.name}>
              {r.kind === 'other' ? r.name : KIND_LABEL[r.kind]}
            </span>
