...
Оценка как в model.holdout_metrics: окт 2025–янв 2026, граница обучения S − 1 сут − 1 ч,
выпуски погоды по horizon_leads.
        ...
        rows.append(
            {
                "month": mon,
                "w_model": round(w, 2),
                "model": (cur.model - cur.y).abs().mean(),
                "turb_avg": (cur.turb - cur.y).abs().mean(),
                "curve": (cur.curve - cur.y).abs().mean(),
                "blend": (blend - cur.y).abs().mean(),
                "days_model_worse_curve": int((daily_model > daily_curve).sum()),
                "days_blend_worse_curve": int((daily_blend > daily_curve).sum()),
                "days": len(daily_model),
            }
        )
